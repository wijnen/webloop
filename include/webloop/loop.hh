...
public:
	...
	// Classes that provide callbacks should inherit from this.
	struct CbBase {};
	...
	struct IoRecord { // {{{
		...
		IoRecord(std::string const &name, UserType *obj, int fd,
				short events, bool (UserType::*r)(),
				bool (UserType::*w)(), bool (UserType::*e)()) :
	...
	struct IdleRecord { // {{{
		CbBase *object;
		Cb cb;
		// Allow initializing this struct from any base class,
		// as long as the cb matches the object.
		template <class Base> IdleRecord(Base *object,
				bool (Base::*cb)()) :
			object(reinterpret_cast <CbBase *>(object)),
			cb(reinterpret_cast <Cb>(cb))
		{}
	}; // }}}
	...
	struct TimeoutRecord { // {{{
		...
		bool operator<(TimeoutRecord const &other) const
		{ return time < other.time; }

		template <class Base> TimeoutRecord(Time const &time,
				Duration interval, Base *object,
				bool (Base::*cb)()) :
...
private:
	struct PollItems { // {{{
		...
		// Empty items that have lower index than num - 1.
		std::set <int> empty_items;

		PollItems(int initial_capacity = 32) :
			data(new struct pollfd[initial_capacity]), num(0),
		...
		std::string print();
		void update_name(IoHandle handle, std::string const &n)
		{ items[handle].set_name(n); }
	}; // }}}
...
public:
	...
	TimeoutHandle add_timeout(TimeoutRecord const &timeout)
	{ timeouts.insert(timeout); return --timeouts.end(); }
	...
	std::set <TimeoutRecord>::const_iterator invalid_timeout() const
	{ return timeouts.end(); }

	IdleHandle invalid_idle() { return idle.end(); }
	std::list <IdleRecord>::const_iterator invalid_idle() const
	{ return idle.end(); }

	void update_name(IoHandle handle, std::string const &n)
	{ items.update_name(handle, n); }
};
